...
| Plant_Function__c | a2yHn000003Km9SIAS (SHA-ASM-Station 4 Final Fit) |
...
- Output: `scoredIdeasJson`, `topIdeaId`, `topIdeaName`, `topConfidenceScore` (87), `topBenchmarkEvidence`
...
- Prompt Template merge fields:
  - `{HQ_Gap}` = 5.0 FTE, `{Quarter}` = Q2 2026
  - `{ProductionContext}` = free text from HQ Target
  - `{Station}` = station name from signal
  - `{ScoredIdeasJson}` = JSON from Step 3
  - `{BenchmarkEvidence}` = evidence string from Step 3
...
- Used by Plant Admin (Jordan) for conversational queries: "What ideas do I have for Station 12?"
- Agent calls `RetrievePlantContextAction` + `MatchIdeaLibraryAction` in response
...
- Force.com Site (not Experience Cloud) exposes callback endpoint publicly over HTTPS
...
|---|---|---|---|
| Alex Chen (`hq.admin@plantiq-demo.com`) | `GWB_HQ_Admin` | Create/edit HQ Targets, view all data read-only | Edit Productivity Ideas |
| Jordan Smith (`jordan.smith@plantiq-demo.com`) | `GWB_Plant_Admin` | Create/edit Productivity Ideas and Edge Signals | Edit HQ Targets |
...
└── create_demo_users.apex          # Alex Chen (HQ) + Jordan Smith (Plant)
```
...
# Then redeploy: sf project deploy start --target-org gwb-demo --source-dir force-app/main/default/customMetadata
```
